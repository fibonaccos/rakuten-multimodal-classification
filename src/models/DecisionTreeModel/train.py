"""Training script for DecisionTreeModel."""
import pandas as pd
import numpy as np
...
def train_model(logger):
    ...
    logger.info(f"Tree depth: {model.get_depth()}")
    logger.info(f"Number of leaves: {model.get_n_leaves()}")
    ...
    train_accuracy = model.score(X_train.values, y_train_encoded)
    logger.info(f"Train accuracy: {train_accuracy:.4f}")
    logger.info(f"Overfitting gap: {train_accuracy - accuracy:.4f}")
    
    if train_accuracy - accuracy > 0.15:
        logger.warning("Potential overfitting detected! Consider adjusting max_depth, min_samples_split, or ccp_alpha.")
    ...
    logger.info("Exporting tree structure...")
    export_tree_structure(model, feature_names, train_config['artefacts']['tree_structure'])
    ...
    # Tree visualization (only for small trees)
    if model.get_n_leaves() < 100:
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(train_config['visualization']['tree_visualization'], dpi=150, bbox_inches='tight')
            plt.close()
            logger.info("Tree visualization saved (limited to depth 3 for readability)")
        except Exception as e:
            logger.warning(f"Could not generate tree visualization: {e}")
    else:
        logger.info("Tree too large for visualization, skipping...")
