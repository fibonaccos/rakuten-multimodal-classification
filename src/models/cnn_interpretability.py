...
def build_activation_model(model, layer_names):
    ...
    return Model(inputs=backbone.input, outputs=layers)


def show_activations(original_img, activations, layer_names, im_name, num_filters=8, layer_pos="surface"):
    plt.figure(figsize=(12, 2 * len(layer_names) + 1))
    plt.subplot(len(layer_names) + 1, num_filters, 1)
    plt.imshow(original_img)
    plt.axis("off")
    plt.title("Originale")

    for i, (acts, lname) in enumerate(zip(activations, layer_names)):
        acts = acts[0]
        for f in range(num_filters):
            plt.subplot(len(layer_names) + 1, num_filters, (i+1)*num_filters + f + 1)
            plt.imshow(acts[:, :, f], cmap="viridis")
            plt.axis("off")
            if f == 0:
                plt.title(lname)
    plt.tight_layout()
    plt.savefig(f"./reports/figures/{layer_pos}_mosaic_{im_name.split('/')[-1]}")  # type: ignore


# ----- Couches de surface -----

layer_names = ["conv1_conv", "conv2_block3_out"]
act_model = build_activation_model(model, layer_names)

for im_name, (pil_img, arr) in zip(paths, images):
    activations = act_model.predict(arr)
    show_activations(pil_img, activations, layer_names, im_name)


# ----- Couches profondes -----

layer_names = ["conv4_block3_out", "conv5_block3_out"]
act_model = build_activation_model(model, layer_names)

for im_name, (pil_img, arr) in zip(paths, images):
    activations = act_model.predict(arr)
    show_activations(pil_img, activations, layer_names, im_name, layer_pos="deep")
